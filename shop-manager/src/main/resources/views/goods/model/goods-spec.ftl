  ...
  <section class="content">
        ...
        <div class="panel-body">
          <div class="navbar navbar-default">
              <form action="" id="search-form2" class="navbar-form form-inline" method="post" onsubmit="return false">
                ...
                <div class="form-group">
	                <button type="submit" onclick="shaixuan()" id="button-filter" class="btn btn-primary pull-right">
	                 <i class="fa fa-search"></i> 筛选
	                </button>
                </div> 
                <button type="submit" onclick="location.href='${ctx}/goods/spec/add'" id="button-filter2" class="btn btn-primary pull-right">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover">
            ...
            <tbody>
             <#list gmsList as list>
                <tr id="tr_specId">

                    <td class="text-center">
                        <input name="selected[]" value="6" type="checkbox">
                    </td>
                    <td class="text-right">${list.specId}</td>
                    <td class="text-left">${list.specName}</td>
                    <td class="text-left">${list.typeName}</td>
                    <td class="text-left">${list.specValue}</td>
                    <td class="text-center">
                        <img src="${ctx}/static/images/yes.png" onclick="changeTableVal('spec','id','25','search_index',this)" width="20" height="20">
                    </td>
                    <td class="text-right">
                        <input class="form-control input-sm" onchange="updateSort('spec','id','25','order',this)" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" size="4" value="${list.specOrder}" type="text">
                    </td>
                    <td class="text-right">                       
                       <a href="${ctx}/goods/spec/add?specId=${list.specId}" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑"><i class="fa fa-pencil"></i></a>

                        <a href="javascript:delfunc(${list.specId});" id="button-delete6" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="删除"><i class="fa fa-trash-o"></i></a></td>
...
<script>
    ...
    function ajax_get_table(form,page){
		cur_page = page; //当前页面 保存为全局变量
            $.ajax({
                type : "POST",
                url:"${ctx}/goods/spec/select="+page,//+tab,
...
<script type="text/javascript">
    function delfunc(specId){
        layer.confirm('确认删除？', {
                btn: ['确定','取消'] //按钮
            }, function(){
                $.ajax({
                    type : 'post',
                    url : "${ctx}/goods/spec/deletes",
                    data : {
                        specId:specId
                    },
                    dataType : 'json',
